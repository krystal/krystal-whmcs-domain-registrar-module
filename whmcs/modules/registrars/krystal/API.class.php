...
namespace Krystal\API1;

use \Krystal\API1\Exceptions\Exception;
use \Krystal\API1\Exceptions\AuthenticationException;
use \Krystal\API1\Exceptions\AuthorisationException;
use \Krystal\API1\Exceptions\FatalAPIException;
use \Krystal\API1\Exceptions\UserInputAPIException;
use \Krystal\API1\Exceptions\HTTPException;
use \Krystal\API1\Exceptions\NotFoundException;
...
class API
{
	...
	public function call($route, $method, $url_params = [], $body = [], $options = [])
	{
		...
		// Set url
		curl_setopt($cURL, CURLOPT_URL, "https://krystal.io/api/1/{$route}?" . http_build_query($url_params));
		...
		// Log the result
		\logModuleCall("krystal", "{$method} {$route}", ["params" => $url_params, "body" => $body, "options" => $options, "headers" => $headers], null, ["response" => $response], $excludedFromLogging);
